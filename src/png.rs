...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let file_slices = file.as_slice();
        Self::try_from(file_slices)
    }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        match ChunkType::from_str(chunk_type) {
            Ok(new_chunk_type) => {
                if let Some(pos) = self.chunks()
                .iter()
                .position(|chunk| *chunk.chunk_type() == new_chunk_type) {
                    Ok(self.chunks.remove(pos))
                } else {
                    Err("Chunk to remove not found.")?
                }
            },
            Err(e) => Err(e)
        }
        ...
        match ChunkType::from_str(chunk_type) {
            Ok(new_chunk_type) => {
                for chunk in self.chunks() {
                    if *chunk.chunk_type() == new_chunk_type {
                        return Some(chunk)
                    }
                }
                None
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        while let Ok(()) = reader.read_exact(&mut buffer) {
            // 4 bytes for data length (already read)
            // + 4 bytes for chunk type
            // + variable-length chunk data
            // + 4 bytes for CRC 
            let data_length = u32::from_be_bytes(buffer);
            let total_bytes_remaining_for_chunk: u64 = (4 + data_length + 4).into();
            let mut remaining_bytes = vec![0u8; usize::try_from(total_bytes_remaining_for_chunk)?];
            reader.read_exact(&mut remaining_bytes)?;

            let chunk_bytes: Vec<u8> = buffer
                .iter()
                .chain(remaining_bytes.iter())
                .copied()
                .collect();

            png_vec.push(Chunk::try_from(chunk_bytes.as_slice())?);
        } 

        Ok(Png::from_chunks(png_vec))
    }
}
